...
function asAmirror(str) {
  ...
  for (var i=0; i<strArray.length; i++) {
    var palabra = strArray[i];
    var strReversed = '';

    for(var j=palabra.length -1; j>=0; j--){
      strReversed += palabra[j];
    }
    reversed.push(strReversed)
  }
  return reversed.join(' ');
} 
...
function capicua(numero){
  ...
  numAStr = numero.split('').reverse().join('');
  // numAStr = numAStr.split('');
  // numAStr = numAStr.reverse();
  // numAStr = numAStr.join('');
  if (numero === numAStr) return "Es capicua";
